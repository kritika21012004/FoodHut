...
const FoodPage = () => {
    ...
    const getFoodDetails = async () => {
        try {
            const res = await axios.get(`http://localhost:8008/api/v1/food/getFood/${params.id}`)
            if (res.data.success) {
                    ...
                    <div className='grid grid-cols-1 md:grid-cols-2 pb-14 gap-8'>
                        ...
                        <div className='bg-red-200/[.3] border rounded p-8 text-black mb-5 ' >
                            ...
                            <div className='flex flex-col sm:flex-row items-center space-y-6 sm:space-y-0 sm:gap-5 sm:mx-auto sm:justify-center'>
                                <button className='bg-white active:scale-90 transition duration-500 transform hover:shadow-x1 shadow-md rounded-full px-8 py-2 text-x1 font-medium text-[#f54748]'>
                                    Favourite
                                </button>
