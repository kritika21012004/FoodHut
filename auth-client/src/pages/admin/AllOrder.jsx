...
const AllOrder = () => {
  ...
  const getAllOrders = async () => {
    try {
      const res = await axios.post('http://localhost:8008/api/v1/order/getorders', {
        ...
        <div className='container mx-auto py-6'>
          <div className='w-full bg-white px-10 py-5 text-black rounded-md'>
            ...
            <div className='flex mt-10 mb-5'>
              <h6 className='font-semibold text-gray-900 text-5xl uppercase w-2/5'>
                Food Details
              </h6>
              <h3 className='text-center font-semibold text-gray-900 text-5xl uppercase w-2/5'>
                Payment
              </h3>
              <h3 className='text-center font-semibold text-gray-900 text-5xl uppercase w-2/5'>
                Date
            ...
            </div>
            {
              ...
              order?.map((order) => (
                <OrderFoods key={order._id} order={order} />
              ))
...
const OrderFoods = ({ order }) => {
  ...
  const handleDelivered = async (id) => {
    ...
    try {
      const res = await axios.post(
        'http://localhost:8008/api/v1/order/delivered',
        {
          userId: user?.user._id,
          orderId: id,
          token: localStorage.getItem('token')
  ...
  return (
    <div className='flex items-center hover:bg-gray-100 -mx-8 px-6 py-5'>
      ...
      <div className='flex justify-center w-1/5 cursor-pointer'>
        <button className='bg-[#f54478] active:scale-90 transition duration-150 transform hover:shadow-xl shadow-md rounded-full px-8 py-2 text-xl font-medium text-white mx-auto text-center' onClick={() => handleDelivered(order?._id)}>Delivered</button>
      </div>
